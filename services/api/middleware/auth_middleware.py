...
# Corrected import path for auth utils (one level up from 'middleware', then into 'utils')
...
# Import error types
from ...utils.error_utils import APIError, format_error_response
from ...db.exceptions import (
    DatabaseError, ConnectionError, QueryError, 
    UserNotFoundError, ApiKeyNotFoundError, ApiKeyInvalidError
)

# Corrected import path for DB functions (two levels up from 'middleware', then into 'db')
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # 2. Check for API Key (if JWT not found/valid and allow_api_key is True)
                ...
                api_key = request.headers.get('X-API-Key') # Prioritize header only
                if api_key:
                    try:
...
def admin_required():
    ...
    # Explicitly capture and return the decorator function generated by require_auth
