...
This module contains the OpenRouter provider implementation for accessing multiple
model providers through a single API.
"""
...
class OpenRouterProvider(LLMProvider):
    """
    OpenRouter API provider implementation for accessing multiple models
    """
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        return {
            ...
            "max_tokens": 200000,  # Uses the max of whatever model is selected
    ...
    def _make_request(self, data: Dict[str, Any], stream: bool = False) -> requests.Response:
        ...
        headers = {
            ...
            "X-Title": os.getenv("OPENROUTER_X_TITLE", "LLM Service")
        }
    ...
    def _process_response(self, response: requests.Response) -> LLMResponse:
        ...
        # Get the actual model used (might be different than requested)
        ...
        return LLMResponse(
            ...
            details={"original_provider": result.get("provider", {}).get("name")}
        )
    ...
    def _process_stream_response(self, 
        ...
        for line in response.iter_lines():
            ...
            try:
                ...
                # Get the model information if available
                if "model" in chunk_data:
                    actual_model = chunk_data["model"]
                
                # Get the provider information if available
                if "provider" in chunk_data:
                    original_provider = chunk_data["provider"].get("name")
        ...
        return streaming_handler.finalize({
            ...
            "details": {"original_provider": original_provider}
